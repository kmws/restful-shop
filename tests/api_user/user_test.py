...
@pytest.mark.parametrize("user_dict, expected_status_codes",
                         [({'email': 'test@mail.com', 'password': 'Test123!'},
                           [201, 400, 204, 204, 204, 204, 200, 204, 204, 200])])
def test_login_activated_user(client, user_dict, expected_status_codes):
    ...
    response = client.get('/api-user/user')
    assert response.status_code == expected_status_codes[6]
    user_dict = {"firstName": "test"}
    response = client.put('/api-user/user', json=user_dict)
    assert response.status_code == expected_status_codes[7]
    response = client.delete('/api-user/user')
    assert response.status_code == expected_status_codes[8]
    response = client.get('/api-user/user')
    assert response.status_code == expected_status_codes[9]
